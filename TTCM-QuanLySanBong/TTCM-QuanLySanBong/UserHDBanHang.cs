...
{
    public partial class UserHDBanHang : UserControl
    {
        public UserHDBanHang()
        {
            InitializeComponent();
            LoadCTHoaDon();
        }
        ...
        void LoadCTHoaDon()
        {
            string querry = "select HD.mahd, HH.tenhh,CTHD.soluongban,PARSENAME(CONVERT(varchar, CAST(CTHD.dongia AS money), 1), 2) as dongia,PARSENAME(CONVERT(varchar, CAST(CTHD.thanhtien AS money), 1), 2) as thanhtien from HoaDon HD, CTHoaDon CTHD, HangHoa HH where HD.maHd=CTHD.mahd and CTHD.mahh = HH.mahh ";
            DataTable data = KetNoi.Istance.ExcuteQuerry(querry);
            dgvCTHoaDon.DataSource = data;
        }
